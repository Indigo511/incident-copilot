...
class SafetyTests(unittest.TestCase):
    def setUp(self):
        self.evidence = FixtureIncidentTools(ROOT / "data/live_incident.json").investigate_unlock_failures("card-unlock-service")
    ...
    def test_old_version_logs_do_not_support_new_version_diagnosis(self):
        self.evidence[2].data["results"][:] = [item for item in self.evidence[2].data["results"] if item["version"] == "v2.3"]
        self.assertEqual(self.report().status, "insufficient_evidence")

    def test_any_release_names_work(self):
        self.evidence[0].data["deployments"][0]["version"] = "release-99"
        rates = self.evidence[1].data
        rates["release-99"] = rates.pop("v2.4")
        for item in self.evidence[2].data["results"]:
    ...
    def test_tiny_sample_abstains(self):
        self.evidence[1].data["v2.4"]["request_count"] = 2
        self.assertEqual(self.report().status, "insufficient_evidence")

    def test_invalid_rates_abstain(self):
        for rate in (-0.1, 1.1, float("nan"), "0.2", True):
            with self.subTest(rate=rate):
                self.evidence[1].data["v2.4"]["system_induced_rate"] = rate
                self.assertEqual(self.report().status, "insufficient_evidence")
    ...
    def test_invalid_requests(self):
        copilot = IncidentCopilot(ROOT / "knowledge", ROOT / "data/live_incident.json")
        ...
        self.assertEqual(copilot.investigate("Check unlock health").data_mode, "synthetic_fixture")
